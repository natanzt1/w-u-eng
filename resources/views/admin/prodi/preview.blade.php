...
<script>
    $(document).ready(function() {
        $("#active").click(function() {
            var slug = $("#slug").val();
            var link = "/administrator/agama/"+slug+"/active"
            $.get(link, function() {
        ...
        $("#nonactive").click(function() {
            var slug = $("#slug").val();
            var link = "/administrator/agama/"+slug+"/nonactive"
            $.get(link, function() {
...
<section class="imagebg unpad">
    <div class="background-image-holder" style="height: 280px;">
        <img alt="background" src="{{ URL::asset('guest/img/gallery/education-6.jpg') }}" />
    </div>
    <div class="container">
        <div class="row">
            <div class="col-sm-4">
                ...
                <div class="masonry">
                    <article class="single-post">
                        <div class="post-heading">
                            <h2 class="post-title">
                                <span>{{$datas[0]->nama_agamabudaya}}</span>
                            </h2>
                            <ul class="entry-meta">
                    ...
                    <div class="blog-comments">
                        <div class="fb-comments" data-href="http://127.0.0.1:8000/id/agamabudaya/detail-agamabudaya/slug" data-width="100%" data-numposts="5"></div>
                    </div>
                </div>
                <!--end masonry-->
            </div>
                ...
                <div class="sidebar boxed boxed--border bg--secondary" style="padding:20px 0 ">
                    <div class="sidebar__widget">
                        <div style="padding: 0 20px">
                            @if($source == "create")
                            <a href="{{route('admin.agamabudaya.active', [$slug])}}"><button class="btn btn-primary" style="width: 100px">Simpan</button></a>
                            @elseif($source == "edit")
                            <form method="POST" action="{{route('admin.agamabudaya.saveUpdate', [$slug])}}">
                                @csrf
                                <h3>Simpan Perubahan?</h3>
                                <input type="hidden" name="nama_agamabudaya" value="{{$datas[0]->nama_agamabudaya}}">
                            ...
                            <input type="hidden" id="slug" value="{{$datas[0]->slug}}">
                                @if($datas[0]->status == 0)
                            <a href="{{route('admin.agamabudaya.edit', [$datas[0]->slug])}}"><button class="btn btn-info" style="width: 100px">Edit</button></a>
