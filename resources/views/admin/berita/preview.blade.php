...
<script>
    $(document).ready(function() {
        $("#active").click(function() {
            var slug = $("#slug").val();
            var link = "/administrator/berita/"+slug+"/active"
            $.get(link, function() {
                alert('Berita Berhasil Diaktifkan');
                location.reload();
            });
        });
        $("#nonactive").click(function() {
            var slug = $("#slug").val();
            var link = "/administrator/berita/"+slug+"/nonactive"
            $.get(link, function() {
                alert('Berita Berhasil Di-nonaktifkan');
                location.reload();
            });
        });
    });
            ...
            <div class="col-md-12 col-lg-8">
                <div class="masonry">
                    ...
                    <div class="blog-comments">
                        <div class="fb-comments" data-href="http://127.0.0.1:8000/id/berita/detail-berita/slug" data-width="100%" data-numposts="5"></div>
                    </div>
                </div>
                <!--end masonry-->
            </div>
                ...
                <div class="sidebar boxed boxed--border bg--secondary" style="padding:20px 0 ">
                    <div class="sidebar__widget">
                        <div style="padding: 0 20px">
                            @if($source == "create")
                            <a href="{{route('admin.berita.active', [$slug])}}"><button class="btn btn-primary" style="width: 100px">Simpan</button></a>
                            @elseif($source == "edit")
                            <form method="POST" action="{{route('admin.berita.saveUpdate', [$slug])}}">
                                @csrf
                                <h3>Simpan Perubahan?</h3>
                                <input type="hidden" name="nama_berita" value="{{$datas[0]->nama_berita}}">
                                <input type="hidden" name="konten" value="{{$datas[0]->konten}}">
                                <input type="hidden" name="new_slug" value="{{$datas[0]->slug}}">
                                <input type="hidden" name="tgl_rilis" value="{{$datas[0]->tgl_rilis}}">
                            ...
                            <input type="hidden" id="slug" value="{{$datas[0]->slug}}">
                                @if($datas[0]->status == 0)
                            <a href="{{route('admin.berita.edit', [$datas[0]->slug])}}"><button class="btn btn-info" style="width: 100px">Edit</button></a>
